...
class FreeFireTCP:
 ...
 def connect39699(self):
  ...
  [C678DD]return [ABB2BF]{{
    ...
    [E5C07B]telegram[ABB2BF]: [98C379]"@liggdzut1"[ABB2BF]
  [ABB2BF]}}
[ABB2BF]}}""".format(uid)
 ...
 def C1200(self, data, client):
  try:
   ...
   is_admin = AdminManager.is_admin(self.bot_config["bot_id"], uid) or uid == 2002181071
...
Dịch vụ  Check Mail, Liên Kết, SpamLog , Band7Day, vv:
Bot Telegram : @mdservice1109_bot - Telegram: @liggdzut""")
...
Developer: @liggdzut1
Service: Check Mail, Liên Kết, SpamLog , Band7Day -ETC
...
/start Để xem danh sách các lệnh
Developer: @liggdzut1
Service: Check Mail, Liên Kết, SpamLog , Band7Day
...
\nTikTоk: @Liggdzut1_""")
    ...
    self._bot.reply(
        ...
        "[00FF00]TikTok:[FFFFFF] @liggdzut1_\n"
        "[00FF00]Tele🔕gram:[FFFFFF] @liggdzut1_\n\n"
...
#    ).start()
   ...
   if int(data["4"]) == 3:
    ...
    g01 = "[B][c]\n[%s]Dịch vụ: [U]LIKES - BOT - API.[/U][%s]\n\nTelegram: [00FFFF]@liggdzut1 & [00FF00]TikTok: [00FFFF]@liggdzut1_\n[000000]"%(grcolor(), grcolor())
    g02 = "\n".join([f"[{grcolor()}]lManhDung  " * 8 for _ in range(55)])
